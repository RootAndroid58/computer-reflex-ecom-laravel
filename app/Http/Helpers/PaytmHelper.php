...
class PaytmHelper{
    ...
    static public function status($data)
    {
        ...
        $paytmParams["body"] = array(
            "mid"          => \Config::get('indipay.paytm.MID'),
			"orderId" 		=> $data['orderId'],
        );
        $checksum = PaytmHelper::generateSignature(json_encode($paytmParams["body"], JSON_UNESCAPED_SLASHES), \Config::get('indipay.paytm.MERCHANT_KEY'));
		...
		if (\Config::get('indipay.testMode') == true) {
			$url = 'https://securegw-stage.paytm.in/v3/order/status';
		} else {
			$url = 'https://securegw.paytm.in/v3/order/status';
		}

        return $paytm->prepare($url, $post_data);
    }
    ...
    static public function prepare($url, $post_data)
    {
        $ch = curl_init($url);
    ...
    {
        $mode = \Config::get('indipay.testMode');
        return $mode ? 'https://securegw-stage.paytm.in/refund/apply':'https://securegw.paytm.in/refund/apply';
    }
